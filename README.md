...
| BOT Mainnet | 677 | Pending | No deployment recorded yet |

The frontend reads deployment state from `deployments/968.json` and `deployments/677.json`. Mainnet writes stay disabled until its contract, source verification, and a live check are recorded in the mainnet manifest.

The contract source is in [`contracts/src/LastlightVault.sol`](contracts/src/LastlightVault.sol). Foundry configuration and contract tests are under `contracts/`. The site is built as static files and can be hosted on GitHub Pages.
