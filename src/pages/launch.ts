...
export function renderLaunch(mainnet: DeploymentManifest = manifests[677]): string {
  ...
  const activeDescription = released
    ? 'Lastlight is officially launched on BOT Chain Mainnet. The contract is verified, read-only mainnet checks passed, and a funded create-to-claim path succeeded on testnet.'
    : testnetVerified
      ? 'The testnet contract and source are verified. A funded create-to-claim path succeeded on BOT Testnet.'
      : testnetDeployed
